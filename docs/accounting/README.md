...
### M3 売上売掛・M4 経費の専用画面、永続化 (DB化)
| 機能 | backend | frontend | テスト | 状態 |
|------|---------|----------|--------|------|
| 売上・売掛 (M3 F-301/302) | `app/api/receivables.py` / `services/tax_calc.py` | `components/ReceivableView.vue` | ✓ | 実装済 (発生主義・内税・取引先別未入金・入金消込) |
| 経費・支払 (M4 F-401/403) | `app/api/expenses.py` | `components/ExpenseView.vue` | ✓ | 実装済 (即時/未払・税区分既定・未払消込) |
| 永続化 / DB化 (N-2) | `app/db.py` / `app/main.py` / `alembic/` | — | ✓ | 実装済 (SQLModel+SQLite, Alembic, 再起動でデータ保持) |

> backend テスト **47 件パス** (ruff/mypy クリーン)、frontend type-check/lint クリーン。
> **永続化**: `DATABASE_URL` (例 `sqlite:///./accounting.db`) を設定すると、起動時に DB から
> ロードし更新後にスナップショット保存。未設定なら in-memory のまま (テストは in-memory)。
> スキーマは Alembic マイグレーション (`alembic upgrade head`) で管理。
> 残: M8 決算整理ウィザード、M9 仕訳CSVインポート/バックアップ。次の基盤候補は `add-auth` (認証・N-4)。
